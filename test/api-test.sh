...
# ║                                                              ║
# ║  Tests core API workflows with expected responses.           ║
...
assert_status "GET /health/live returns 200" "200" "$LIVE_STATUS"
HEALTH_STATUS=$(curl -s -o /dev/null -w "%{http_code}" "$BASE_URL/health/ready")
assert_status "GET /health/ready returns 200" "200" "$HEALTH_STATUS"
HEALTH_BODY=$(curl -s "$BASE_URL/health/ready")
...
EXAMINER_ID=$(curl -s "$BASE_URL/users" \
  -H "Authorization: Bearer $TOKEN" | python3 -c "import sys,json; users=json.load(sys.stdin)['data']; print([u['id'] for u in users if u['username']=='examiner'][0])" 2>/dev/null)
...
ALICE_ID=$(curl -s "$BASE_URL/users" \
  -H "Authorization: Bearer $TOKEN" | python3 -c "import sys,json; users=json.load(sys.stdin)['data']; print([u['id'] for u in users if u['username']=='alice'][0])" 2>/dev/null)
