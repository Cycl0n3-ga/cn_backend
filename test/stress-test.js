...
async function runStressTest() {
  ...
  for (const scenario of scenarios) {
    ...
    console.log(`👥 ${scenario.connections} connections × ${scenario.duration}s\n`);
  ...
  for (const r of results) {
    ...
    console.log(`   Latency — P50: ${r.latency.p50}ms | P95: ${r.latency.p97_5}ms | P99: ${r.latency.p99}ms | Max: ${r.latency.max}ms`);
    console.log(`   Errors: ${r.errors} | Timeouts: ${r.timeouts}`);
    console.log(`   Status — 2xx: ${r.statusCodes['2xx']} | 4xx: ${r.statusCodes['4xx']} | 5xx: ${r.statusCodes['5xx']}`);
    ...
    if (r.errors > 0) {
      console.log(`   ⚠️  ${r.errors} connection errors — may indicate resource exhaustion`);
    }
    if (r.timeouts > 0) {
      console.log(`   ⚠️  ${r.timeouts} timeouts — server may be overloaded`);
    }
    if (r.statusCodes['5xx'] > 0) {
      console.log(`   🔴 ${r.statusCodes['5xx']} server errors (5xx) — investigate immediately`);
    }
